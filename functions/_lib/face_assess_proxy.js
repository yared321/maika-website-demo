/** @typedef {import("./cf_http.js").CfEnv} CfEnv */
...
/**
 * Proxies `/api/face-assess/*` → MAIKA_RPPG_UPSTREAM.
...
export async function proxyFaceAssess(request, env) {
  ...
  const normalizedPath = upstreamPathTail.startsWith("/")
    ? upstreamPathTail
    : `/${upstreamPathTail}`;
  ...
  try {
    ...
    const outBody = await upstreamRes.arrayBuffer();
